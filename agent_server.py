...
<<<<<<< HEAD
# Config – MATCHES YOUR COOLIFY ENVS
...
def scrape():
    ...
    try:
        ...
        with Camoufox(headless=HEADLESS, humanize=True) as browser:
            ...
            # keep as networkidle for now; you can change to 'load' if timeouts persist
...
def agent():
    if not BROWSER_USE_AVAILABLE:
        return jsonify(
            {"error": "browser-use not installed (add to requirements.txt)"}
        ), 500
...
<<<<<<< HEAD
            run_browser_agent(task, model_name, max_steps, headless)
        )
        loop.close()
...
async def run_browser_agent(task: str, model_name: str, max_steps: int, headless: bool):
    ...
    # browser-use ≥ 0.2 uses max_actions_per_step
=======
